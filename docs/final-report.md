...
## 10. v2 Professional Rebuild (Month 6 completion)

**What changed after the first evaluation:** the platform was consolidated from a
single-file dashboard into a full **microservices product**:

- **Control plane** (`web/`): landing page, live dashboard, public status page, admin
  console — one Express service on `:3100`, with a single `/api/aggregate` endpoint.
- **API layer v2** (`api/`): per-key rate limiting, audit log, cost-forecast and
  monitoring endpoints, Swagger UI on `:4000`.
- **Monitoring v2** (`orchestration/monitor.js`): multi-region probes (region tags),
  incident opening/closing with downtime duration, alert delivery (Slack/webhook/email),
  self-heal via GitHub deploy dispatch, latency-threshold degradation detection.
- **Analytics core** (`packages/common/analytics.js`): uptime %, p50/p90/p95/p99,
  error rate, and a composite 0–100 health score per cloud.
- **Anomaly detection** (`intelligence/anomaly.js`): robust median+MAD z-score detector
  that runs live in the monitor and in the browser — in addition to the CI
  Isolation Forest (`intelligence/ml/predict.py`).
- **Failover scoring** (`intelligence/failover.js`): rank clouds by health, then
  latency, then carbon intensity; deterministic + unit-tested.
- **Report generator** (`orchestration/report.js`): daily/weekly reports persisted to
  `health_reports` and optionally emailed (Resend free tier).
- **Infra**: schema v2 (10 tables incl. incidents, alerts, monitor_targets,
  failover_events, audit_log, health_reports), Docker Compose for one-command local
  runs, Terraform for optional provisioning.
- **Docs**: deployment playbook, Oracle VM guide, future-scope roadmap, status page.

**Verified live:** ~850 health checks in Supabase; the dashboard computes render
(score 85) vs vercel (score 93) and routes to vercel; the anomaly detector flagged
render's cold-start latency spikes; all 30 unit tests pass.

### v2.1 — the real-time layer (added for "make it a real product")

- **Server-Sent Events** (`GET /api/stream`) push a fresh snapshot to every open
  dashboard every 10 s — the screen updates itself as clouds change, no refresh.
- **Live on-demand probes**: the portal itself hits every cloud's `/health` every
  10 s (latency + HTTP status) alongside the 5-min monitors.
- **Live event feed** (probes / routing decisions / incident changes), **SLA
  tracking** vs a 99.9% target, **24h latency trend arrows**, **per-cloud
  sparklines**, **embeddable uptime badges** (`/badge/{cloud}.svg`), **CSV/JSON
  export**, and a dark/light theme.
...
- Cloudflare Workers docs · Supabase docs · Open Policy Agent docs ·
  ...
  - Cloud Run free tier / pricing: https://cloud.google.com/run/pricing
